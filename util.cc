...
float **create_array(const int row, const int col) {
  float **p;
  p = new float *[row];
  for (int i = 0; i <= row; ++i) p[i] = new float[col];
  return p;
}
...
void read_file_into_2d_array(const string file_name, const int row,
                             const int col, float **data) {
...
void find_max_value_index(float *p, float *q, float d, const int K,
                          float &delta_val, int &psi_idx) {
  float tmp = 0.0;
  int max_index = 0;

  // p[0]: last state is 0
  // q[0]: last state is 0, this states is i
  // d: the emission probability of state i observe obs_seg[j]
